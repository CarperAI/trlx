...
def buil_layer_mapping(layer):
    mapp = {
        ...
        f'model.language_model.encoder.layers.{layer}.mlp.dense_h_to_4h_2.weight': f'model.layers.{layer}.mlp.down_proj.weight',
        f'model.language_model.encoder.layers.{layer}.mlp.dense_4h_to_h.weight': f'model.layers.{layer}.mlp.up_proj.weight',
    }
    return mapp

llama_state_dict = AutoModelForCausalLM.from_pretrained("pvduy/llama-30b", cache_dir="../NeMo-1.17.0/examples/nlp/language_modeling/checkpoint/").half()
llama_state_dict = llama_state_dict.state_dict()

total_layers = 60
total_tp = 4
hidden_dim = 6656
ffn_hidden_dim = 17920
...
def mapping(tp_idx):
    nemo_state_dict = torch.load(f"llama-nemo-30b/mp_rank_0{tp_idx}/model_weights.ckpt")
...
for tp_idx in range(total_tp):
    ...
    if not os.path.exists(f"llama-nemo-30b-converted/mp_rank_0{tp_idx}"):
        os.makedirs(f"llama-nemo-30b-converted/mp_rank_0{tp_idx}")
    torch.save(nemo_state_dict, f"llama-nemo-30b-converted/mp_rank_0{tp_idx}/model_weights.ckpt")
