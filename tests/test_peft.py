...
ALL_TEST_COMBINATIONS = [
    ...
    and (
        MODEL_TASK_TYPE[model_path] != SEQ2SEQ or peft_type == PeftType.LORA
    )  # Skip some tests due to implementation problems with peft 0.3.0 and Seq2Seq
]
...
class TestPeft(unittest.TestCase):
    ...
    def _check_that_models_are_equivalent(self, model1, model2, training_type, test_hydra=False):
        ...
        if training_type != SFT:
            self.assertTrue(
                torch.equal(
                    model1(**self.inputs, return_dict=True).value, model2(**self.inputs, return_dict=True).value
                )
            )
