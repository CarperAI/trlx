...
default_config = yaml.safe_load(open("examples/experience_demo/configs/ppo_config_encoder_decoder.yml"))
...
def encoder_decoder_experience_fn(trainer, batch):
    ...
    for i in range(batch_size):
        first_run_strs[i] = prompt_repeat(str(digits[i]))
    ...
    first_run_data, first_run_stats = trainer.orch.generate_and_calc_logprobs(first_run_batch, max_new_tokens=8)
    ...
    for i in range(batch_size):
        second_run_strs[i] = prompt_answer(first_run_str_outputs[i])
