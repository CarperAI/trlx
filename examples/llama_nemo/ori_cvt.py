...
INPUT_FOLDER = "/fsx/home-uwu/in-llama-nemo-7b" # NeMo initial checkpoint folder
OUTPUT_FOLDER = "/fsx/home-uwu/ori-llama-nemo-7b" # NeMo converted checkpoint folder with llama weights
...
def load_nemo_state_dict(tp_idx):
    if TOTAL_TP == 1:
        return torch.load(f"{INPUT_FOLDER}/model_weights.ckpt", map_location="cpu")
    return torch.load(f"{INPUT_FOLDER}/mp_rank_0{tp_idx}/model_weights.ckpt", map_location="cpu")
...
def map_weights(tp_idx):
    ...
    nemo_state_dict['model.language_model.embedding.word_embeddings.weight'] = model_state_dict['model.embed_tokens.weight'][tp_idx * EMBEDDING_DIM : (tp_idx + 1) * EMBEDDING_DIM, :]
    ...
    nemo_state_dict['model.language_model.output_layer.weight'] = model_state_dict['lm_head.weight'][tp_idx * EMBEDDING_DIM : (tp_idx + 1) * EMBEDDING_DIM, :]
