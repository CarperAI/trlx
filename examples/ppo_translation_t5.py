...
if __name__ == "__main__":
    ...
    def reward_fn(samples: List[str], prompts: List[str], outputs: List[str]):
        ...
        scores = comet_metric.compute(
                    ...
                    sources=[original['src'] for original in original_sents])["scores"]

        return scores


    train_dataset = load_dataset("wmt16", "de-en", split='train', cache_dir="/home/aiscuser/dev/trlx_mrt/examples/notebooks/data", streaming=True)
    valid_dataset = load_dataset("wmt16", "de-en", split='validation', cache_dir="/home/aiscuser/dev/trlx_mrt/examples/notebooks/data", streaming=False)
    ...
    for i in tqdm(range(len(val_original_src_dataset))):
        key = tokenizer.decode(
            tokenizer(val_src_dataset[i], truncation=True, max_length=max_length, add_special_tokens=False)["input_ids"],
            skip_special_tokens=True,
        )  # get prompt like trlx's prompt
