...
def main(hparams={}, model_name="EleutherAI/gpt-j-6B", dataset="nomic-ai/gpt4all_prompt_generations"):
    ...
    config = config.evolve(
        ...
        optimizer=dict(kwargs=dict(lr=2e-4)),
        scheduler=dict(kwargs=dict(eta_min=2e-4)),
        method=dict(
            gen_kwargs=dict(
                max_new_tokens=256,
            )
    ...
    gpt4all = load_dataset(dataset, split="train")
    gpt4all = [(x["prompt"], x["response"]) for x in gpt4all]
    ...
    zs_rewrite = ["<p> Rewrite the input into a <i>positive</i> review. </p>" + x["text"][:1024] for x in bad_reviews]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="nomic-ai/gpt4all_prompt_generations")
