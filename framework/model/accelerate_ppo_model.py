...
class AcceleratePPOModel(AccelerateRLModel):
    ...
    def learn(self, log_fn = None, save_fn = None, eval_fn = None):
        ...
        iter_count = 0
        for epoch in tqdm(range(self.config.train.epochs), disable=not self.accelerator.is_local_main_process):
            for batch in tqdm(rollout_loader, disable=not self.accelerator.is_local_main_process):
                ...
                self.post_backward_callback(epoch, iter_count, batch, rewards)

                self.accelerator.wait_for_everyone()
                
            self.post_epoch_callback(epoch, iter_count)
            self.accelerator.wait_for_everyone()