...
class PreTrainedModelWrapper(nn.Module, transformers.utils.PushToHubMixin):
        ...
        **kwargs,
        ...
        Args:
            ...
            peft_config (peft.PeftConfig or dict, *optional*): The peft configuration to create a peft adapter.
                This is only useful when creating a new peft adapter, not when loading it. To
                load an already trained peft adapter, set pretrained_model_name_or_path to the directory containing
        ...
        if isinstance(pretrained_model_name_or_path, str):
            ...
            base_model = None
            if is_peft_available():
                # Check if there is a remote peft adapter
                try:
    ...
    def post_init(self, *args, **kwargs):
        ...
        if self.peft_type:
            ...
            # use the interface of the underlying transformers model instead.
