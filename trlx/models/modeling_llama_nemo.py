...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.n_local_heads = args.n_heads // parallel_state.get_tensor_model_parallel_world_size()
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        h = x + self.attentiontensor_parallel.forward(self.attention_norm(x), start_pos, freqs_cis, mask)
...
class Llama(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.tok_embeddings = tensor_parallel.ParallelEmbedding(params.vocab_size, params.dim, init_method=lambda x: x)
