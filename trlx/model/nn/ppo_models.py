...
class ModelBranch(PreTrainedModel):
    # TODO(dahoas): config and model_trunk are redundant
...
class ModelBranchWithValue(PreTrainedModel):
    # TODO(dahoas): config and model_trunk are redundant
    def __init__(self, config, model_trunk, num_layers_unfrozen):
        super().__init__(config)
    ...
    def forward(self, input_ids, **x):
        ...
        # Compute values
        last_hidden = output.hidden_states[-1].squeeze()
        ...
        # Get size of last hidden state
        output_shape = all_hidden_states[-1].size()
        ref_output = self.branch_forward(input_hidden_state, output_shape, **x)
        ref_logits = ref_output.logits

        return logits, v, ref_logits

class PretrainedHydraModel(PreTrainedModel):
    def __init__(self, model_path, num_layers_unfrozen):
        hf_config = AutoConfig.from_pretrained(model_path)
        super().__init__(hf_config)

        self.gpt = AutoModelForCausalLM.from_pretrained(model_path)

        if hasattr(self.gpt, "hidden_size"):
            self.n_embd = self.gpt.config.hidden_size
        ...
        # Defined by the main trunk
        self.num_layers_frozen = len(self.gpt.transformer.h) - self.num_layers_unfrozen

        self.ln_f = deepcopy(self.gpt.transformer.ln_f)
        self.lm_head = deepcopy(self.gpt.lm_head)
        self.h = deepcopy(
                            nn.ModuleList(list(self.gpt.transformer.h))[-num_layers_unfrozen:]
                         )
    ...
    def forward(self, input_ids, **x):
        return_dict = x.get("return_dict")
        output_hidden_states = x.get("output_hidden_states")
        x["return_dict"] = True
        x["output_hidden_states"] = True

        output = self.gpt(input_ids, **x)
        x["return_dict"] = return_dict
        ...
        # Compute values
        last_hidden = output.hidden_states[-1]#.squeeze()
        v = self.v_head(last_hidden).squeeze(-1)
...
class GPTHydraHeadWithValueModel(PreTrainedModel):
    """The GPTHeadWithValueModel class implements a GPT-type language model with a secondary, scalar head."""

    def __init__(
        self, config: Union[PretrainedConfig, str], num_layers_unfrozen: int = -1
    ):
        pretrained_config = AutoConfig.from_pretrained(config)
        ...
        if num_layers_unfrozen > 0:
            ...
            self.ref_ln_f = deepcopy(self.gpt.transformer.ln_f)
            self.ref_lm_head = deepcopy(self.gpt.lm_head)
            self.ref_h = deepcopy(
                                nn.ModuleList(list(self.gpt.transformer.h))[-num_layers_unfrozen:]
                            )

            # Turning off grad saves memory
            for block in self.ref_h:
                for parameter in block.parameters():
                    parameter.requires_grad = False

            for parameter in self.ref_lm_head.parameters():
                parameter.requires_grad = False

            for parameter in self.ref_ln_f.parameters():
                parameter.requires_grad = False
    ...
    def ref_forward(
        ...
        if past_key_values is None:
            past_key_values = tuple([None] * len(self.ref_h))
        ...
        all_hidden_states = () if output_hidden_states else None
        for i, (block, layer_past) in enumerate(zip(self.ref_h, past_key_values)):
            # Shift block index by number of frozen layers
            i = i + self.num_layers_unfrozen
        ...
        hidden_states = self.ref_ln_f(hidden_states)
        ...
        lm_logits = self.ref_lm_head(hidden_states)
