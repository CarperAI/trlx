...
from trlx.data.ppo_types import RewardFnInput


def train(
    model_path: Optional[str] = None,
    reward_fn: Optional[Callable[[RewardFnInput], List[float]]] = None,
    experience_fn: Optional[Callable] = None,
    dataset: Optional[Iterable[Tuple[str, float]]] = None,
    prompts: Optional[List[str]] = None,
    eval_prompts: Optional[List[str]] = None,
    metric_fn: Optional[Callable[[RewardFnInput], Dict[str, List[float]]]] = None,
    ...
    Args:
        ...
        reward_fn (Optional[Callable[RewardFnInput, List[float]]]):
            Function to rate batches of generated samples. Its arguments are either
            ('trajectories') or ('samples') or (`samples`, `prompts`, `outputs`) 
            and the return is a list of `rewards` per each sample
        experience_fn (Optional[Callable]): Trajectory description, default None means just run prompts through model
        ...
        metric_fn (Optional[Callable[[List[str], List[str], List[str]], Dict[str, List[float]]]]):
            Function to compute statistics on batches of gnerated samples. 
            Its arguments are the same as in `reward_fn`, but the return is dictionary with keys
